...
export class ModalWindow {
  ...
  createWindow(): HTMLDivElement {
    ...
    const title: HTMLHeadingElement = elementGenerator.createHeading('h2', {
    ...
    phoneNumber.addEventListener(
      'focusout',
      () => {
        this._validationForm.blur(phoneNumber, regExprsValid.phone);
      },
      { once: true }
    );
    const phoneDiv: HTMLDivElement = elementGenerator.createDiv({ className: 'check-valid' });
    this._validationForm.mapFormField.set(phoneNumber, () =>
      this._validationForm.checkValidData(phoneNumber, regExprsValid.phone)
    );
    ...
    this._validationForm.mapFormField.set(address, () =>
      this._validationForm.checkValidData(address, regExprsValid.address)
    );
    addressDiv.append(address);

    const email: HTMLInputElement = elementGenerator.createInput('email', {
      className: 'input',
      placeholder: 'E-mail',
      pattern: '^[a-z0-9._%+-]+@[a-z0-9-]+[.][a-z]{2,4}$',
    });
    email.addEventListener('focusout', () => this._validationForm.blur(email, regExprsValid.email), {
      once: true,
    });
    ...
    const titleCard: HTMLHeadingElement = elementGenerator.createHeading('h2', {
      className: 'window_title',
    ...
    const numberCard: HTMLInputElement = elementGenerator.createInput('text', {
      placeholder: 'Card number',
      className: 'input',
      pattern: '[0-9]{16}',
    });
    numberCard.addEventListener('input', () => this._validationForm.validNumberCard(numberCard, imgCard));
    numberCard.addEventListener(
      'focusout',
      () => {
    ...
    this._validationForm.mapFormField.set(numberCard, () =>
      this._validationForm.showNumberMessage(numberCard, imgCard, cardBlock)
    );
    ...
    const dataInput: HTMLInputElement = elementGenerator.createInput('text', {
    ...
    const cvvInput: HTMLInputElement = elementGenerator.createInput('text', {
      placeholder: 'Code',
      className: 'input',
      pattern: '[0-9]{3}',
    });
