...
export class PromoCodeCart {
  ...
  constructor(oldPriceHTML: HTMLParagraphElement) {
    ...
    this._inputPromo = elementGenerator.createInput('text', {placeholder: 'Enter promo code', className: 'summary-cart_input'});
  ...
  private checkInputPromo(event: Event) {
    ...
    const promo: IPromoCode | undefined = this._promoCodes.find((value) => value.id.toLowerCase() === input.value.toLowerCase());
