...
export class SingleProduct {
  ...
  constructor(item: IProduct, isIncrease: boolean) {
    ...
    this._tagList = {
      ...
      addToCart: elementGenerator.createParagraph({className: 'button add-to-cart',text: this._isIncrease ? '+' : 'Add to cart'}),
      removeFromCart: elementGenerator.createParagraph({className: 'button remove-from-cart', text: this._isIncrease ? '-' : 'Remove from cart'}),
  ...
  protected removeProduct(id: string): void { //todo remove return
    const cartLocal: Array<ILocalStorageproduct> = this._localStorage.getLSCart();
    let t = -1;
  ...
  public createProduct(): HTMLDivElement {
    ...
    product.append(this.tagList.title, this.tagList.description, this.tagList.category, this.tagList.brand, this.tagList.rating,
                    this.tagList.stock, this.tagList.price, this.tagList.discount, this.tagList.goToSingle
                  )
    return product;
  }  
